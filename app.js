...
app.route('/api/sendnotification')
    .post(function (req, res) {
        ...
        db.query(sql, (err, result) => {
            ...
            const sql2 = `SELECT * FROM Account_Info WHERE user_name = '${result[0].user_id}'`;
...
app.route('/api/users/:user')
    ...
    //Deletes a specific user along with all of its plants and the data for each plant.
...
app.route('/api/users/:user/plants')
    .get(authJwt, (req, res) => {
        const sql = `SELECT  a.user_id, p.plant_id, plant_name, plant_species, notifications
        FROM Account_Info a
...
app.route('/api/users/:user/plants/:plant')
    .get(authJwt, (req, res) => {
        const sql = `SELECT  a.user_id, p.plant_id, plant_name, plant_species, notifications, water_min, water_max
             FROM Account_Info a
             LEFT JOIN Plant_Info p
                  ON a.user_id = p.user_id 
    ...
    //Deletes a plant from a specific user and also deletes the plants readings.
    .delete(authJwt, (req, res) => {
        ...
        db.query(sql1, (err, result1) => {
            if (!err) {
                ...
                db.query(sql2, (err, result2) => {
                    if (!err) {
                        console.log("Deleted summary; " + JSON.stringify([result2, result1]));
                        res.status(200).send({output: 'success'})
                    } else {
                        res.status(400).send({error: err.sqlMessage});
                    }
